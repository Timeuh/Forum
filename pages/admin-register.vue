<script setup lang="ts">
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const registerResponse = await register(pseudo.value, password.value, repeatedPassword.value, true);
...
<template>
  <section class="flex h-screen w-full bg-slate-900 text-gray-200">
    ...
    >
      <h2 class="text-3xl font-semibold">Créer un compte administrateur</h2>
